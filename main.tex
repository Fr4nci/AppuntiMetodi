...
\begin{document}
	...
	\begin{definition}[punto di accumulazione]
		...
		$$
		\forall \varepsilon > 0, \exists v \neq \underline{v}: v \in A \text{ e } || v - \underline{v} || < \varepsilon.
		$$
	\end{definition}
	\begin{remark}
	...
	\begin{equation*}
		...
		\end{bmatrix}
	\end{equation*}
	Si osserva dunque che ogni $T(V_n) \subset V_n$, siccome restringendoci al sottospazio $V_n$ generato dai vettori $\{ e_1, \ldots, e_n \}$ avremo che il vettore $T(v) \in V_n$ se $v \in V_n$. Adesso, utilizzando il fatto che se $\exists \lambda \in \mathbb{C} : Tv = \lambda v$ allora
	$$
		x_k = \lambda^{-1} (e_k, Tv) = \lambda^{-1} (e_k, \sum_{i=1}^n \frac{x_i}{\sqrt{i}} \sum_{l=1}^i e_l) = \lambda^{-1} (e_k, \sum_{i=k}^n \frac{x_i}{\sqrt{i}} \sum_{l=1}^i e_l) = \lambda^{-1} \sum_{i=k}^n \frac{x_i}{\sqrt{i}}
	$$
	che chiaramente ha soluzione solamente se $k=1$, per cui risulta che $\lambda = 1$ o $x_1 = 0$, mentre per $k \geq 2$ avremo che $x_k = 0$.
